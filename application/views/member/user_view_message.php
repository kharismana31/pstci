										...
										<div class="panel panel-transparent">
											<div class="panel-heading">
												<div class="panel-title">
													View your messages inbox here
												</div>
												<div class="btn-group pull-right m-b-10">
													<button class="btn btn-default" id="delete_check"> Delete Mesage <span class="fa fa-trash"></span></button>
													<a type="button" class="btn btn-default" href="<?php echo site_url('member/C_message/create');?>">Create new Message <span class="fa fa-pencil"></span></a>
...
<script type="text/javascript">
	$(document).ready(function(){
		table = $('#Data_table').DataTable({
			...
			"autoWidth": false,
			"ajax": {
				"url": '<?php echo site_url('member/C_message/table_list'); ?>',
				"type": "POST"
			},
			"columns": [
			{"data": "id",width:0},
		...
		$('#delete_check').click(function(){
			...
			if(id.length === 0){
				...
				modalConfirm(function(confirm){
					if(confirm){
						$.ajax({
							url:'<?php echo site_url('member/C_message/delete_check')?>',
							method:'POST',
							data:{id_message:id},
							success:function()
							{
	...
	function view_messages(id)
	{
		...
		$.ajax({
			url : "<?php echo site_url('member/C_message/get_id_messages/')?>/" + id,
			type: "POST",
			dataType: "JSON",
			success: function(data)
			{
				...
				if (data.status === 'a') {
					$('[name="status"]').val("Sending By Admin");
				}
				else{
					$('[name="status"]').val("Sending By Member");
				}
				
				$('[name="date"]').val(data.date);
				
				$('.m-title').text('View Messages '+ data.name + '');
