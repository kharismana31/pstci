										...
										<div class="panel panel-transparent">
											<div class="panel-heading">
												...
												<div class="btn-group pull-right m-b-10">
													<button class="btn btn-default" id="delete_check">Delete Message <span id="" class="fa fa-trash"></span></button>
...
<script type="text/javascript">
	$(document).ready(function(){
		table = $('#Data_table').DataTable({
			...
			"autoWidth": false,
			"ajax": {
				"url": '<?php echo site_url('member/C_inbox/table_list'); ?>',
				"type": "POST"
			},
			"columns": [
			{"data": "id",width:0},
		...
		$('#delete_check').click(function(){
			...
			if(id.length === 0){
				...
				modalConfirm(function(confirm){
					if(confirm){
						$.ajax({
							url:'<?php echo site_url('member/C_inbox/delete_check')?>',
							method:'POST',
							data:{id_message:id},
							success:function()
							{
	...
	function view_messages(id)
	{
		...
		$.ajax({
			url : "<?php echo site_url('member/C_inbox/get_id_messages/')?>/" + id,
			type: "POST",
			dataType: "JSON",
